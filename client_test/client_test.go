...
func prep_update (period int){
	Update_FULL[period] = monitor.ClientUpdate{
		...
		PoMs: CON_FULL[period],
		MonitorID: ctx_monitor[0].Config.Signer,
		Period: fmt.Sprint(period),
	}
}
...
func Test_MG_Period2(t *testing.T){
	// period 2: all loggers are behaving correctly, 1 ca is behaving correctly, 1 is not responding, 1 is split-world, 1 logger is split-world, 1 monitor is protecting the split-world logger
